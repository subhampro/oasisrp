...
local screenPosY = 0.887                -- Y coordinate (top left corner of HUD)
...
local seatbeltColorOn = {90, 161, 95}     -- Color used when seatbelt is on
local seatbeltColorOff = {255, 255, 255}      -- Color used when seatbelt is off

-- CRUISE CONTROL PARAMETERS
local cruiseInput = 246 --137  CAP                    -- Toggle cruise on/off with CAPSLOCK or A button (controller)
...
local plateColorText = {62, 172, 61} 
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		--PLACA DE VEHICULO EN HUD --  PLATE IN VEHICLE HUD --
        ...
        drawTxt(timeText, 4, locationColorText, 0.40, screenPosX -0.15, screenPosY -0.09)
        

        -- Display Location and time when in any vehicle or on foot (if enabled)
        if pedInVeh or locationAlwaysOn then
            
			--PLACA DE VEHICULO -- PLATE VEHICLE
		    drawTxt(PlateVeh, 4, plateColorText, 0.35, screenPosX +0.00, screenPosY +0.06)
            
            -- Display heading, street name and zone when possible
            drawTxt(locationText, 4, locationColorText, 0.40, screenPosX, screenPosY + 0.083)
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if (GetPedInVehicleSeat(vehicle, -1) == player) then
                    ...
                    if IsControlJustReleased(0, cruiseInput) and (enableController or GetLastInputMethod(0)) then
                        --exports['mythic_notify']:SendAlert('inform', 'Acabas de Poner/Quitar el limitador de velocidad',6000)
                ...
                if ShouldUseMetricMeasurements() then
                    ...
                    drawTxt(("%.2d"):format(math.ceil(speed)), 2, speedColor, 0.50, screenPosX + 0.035, screenPosY + 0.028)
                    drawTxt("KPH", 4, speedColorText, 0.35, screenPosX + 0.052, screenPosY + 0.035) 
                else
                    -- Get vehicle speed in MPH and draw speedometer
                    local speed = currSpeed*2.23694
                    local speedColor = (speed >= speedLimit) and speedColorOver or speedColorUnder
                    drawTxt(("%.2d"):format(math.ceil(speed)), 2, speedColor, 0.50, screenPosX + 0.035, screenPosY + 0.028) --  screenPosX + 0.000, screenPosY + 0.025)
                    drawTxt("MPH", 4, speedColorText, 0.35, screenPosX + 0.052, screenPosY + 0.035) 
                end
                
                -- Draw fuel gauge

                local fuelColor = (currentFuel >= fuelWarnLimit) and fuelColorOver or fuelColorUnder
                drawTxt(("%.2d"):format(math.ceil(currentFuel)), 2, fuelColor, 0.50, screenPosX + 0.000, screenPosY + 0.028) --  screenPosX + 0.055, screenPosY + 0.025)
                drawTxt("FUEL", 4, fuelColorText, 0.35, screenPosX + 0.018, screenPosY + 0.0350) 

                --Draw cruise control status
				
                local cruiseColor = cruiseIsOn and cruiseColorOn or cruiseColorOff
                drawTxt("CRUISE", 4, cruiseColor, 0.35, screenPosX + 0.035, screenPosY + 0.060)

                drawTxt(" HP : "..vehenground , 4 , fuelColorText , 0.40, screenPosX - 0.002, screenPosY + 0.006 )
                ...
                if vehicleClass ~= 8 then
                    ...
                    drawTxt("BELT", 4, seatbeltColor, 0.35, screenPosX + 0.062, screenPosY + 0.060)
...
SetTextProportional(0)
	   SetTextDropShadow(4, 100, 100, 100,255)
	
    SetTextEdge(1, 100, 100, 100, 255)
    SetTextOutline()
