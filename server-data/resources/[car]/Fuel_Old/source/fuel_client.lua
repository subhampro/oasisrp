...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			if GetAmmoInPedWeapon(ped, WEAPON_PETROLCAN) - fuelToAdd * 100 >= 0 then
				currentFuel = oldFuel + fuelToAdd

				SetPedAmmo(ped, WEAPON_PETROLCAN, math.floor(GetAmmoInPedWeapon(ped, WEAPON_PETROLCAN) - fuelToAdd * 100))
			else
				isFueling = false
			end
		else
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetSelectedPedWeapon(ped) == WEAPON_PETROLCAN then
							...
							if GetAmmoInPedWeapon(ped, WEAPON_PETROLCAN) < 100 then
								canFuel = false
							end
						end
				...
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, WEAPON_PETROLCAN) then
							...
							if IsControlJustReleased(0, 38) then
								GiveWeaponToPed(ped, WEAPON_PETROLCAN, 100, false, true)
							...
							if Config.UseESX then
								local refillCost = Round(Config.RefillCost * (1 - GetAmmoInPedWeapon(ped, WEAPON_PETROLCAN) / 100))
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										...
										if IsControlJustReleased(0, 38) then
											...
											SetPedAmmo(ped, WEAPON_PETROLCAN, 100)
										end
									else
