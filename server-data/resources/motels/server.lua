...
Start = function(...)
  ...
  if not ret then print('\27[31m['..res.."]\27[0m Unauthorized usage.")
    ...
    if motels.__VERSION then
      if Config.__VERSION then
        if motels.__VERSION ~= Config.__VERSION then
          print('\27[32m['..res.."]\27[0m You aren't using the latest update. [\27[31m"..Config.__VERSION.."\27[0m / \27[32m"..motels.__VERSION.."\27[0m]")
        end
      else
        print('\27[32m['..res.."]\27[0m You aren't using the latest update. [\27[31mUNKNOWN\27[0m / \27[32m"..motels.__VERSION.."\27[0m]")
      end
    else
...
ESX.RegisterServerCallback(res..':getStartData', function(source, cb) while not motels.ready do Citizen.Wait(0); end cb(motels.getStartData(source)); end)
