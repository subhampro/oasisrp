...
RegisterNetEvent('removetatslul')
AddEventHandler('removetatslul', function()
    ClearPedDecorations(PlayerPedId())
end)

function ShowTatMenu()
  ESX.UI.Menu.Open(
	'default', GetCurrentResourceName(), 'tattoo',
	{
		title    = 'Tattoo Shop',
		align    = 'bottom-right',
		elements = {
			{label = 'Select Tattoo', value = 'gettattoo'},
			{label = 'Remove All Tattoos <span style="color:green;">$'.. Config.RemovalPrice ..'</span>', value = 'removetattoo'}
		}
	},
	function(data, menu)
		local val = data.current.value
		
		if val == 'gettattoo' then
			OpenShopMenu()
		elseif val == 'removetattoo' then
