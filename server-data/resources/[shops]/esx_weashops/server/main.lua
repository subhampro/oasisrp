...
AddEventHandler('esx_weashop:buyCaricatore', function(zone)
  ...
  if zone=="BlackWeashop" then
    if xPlayer.get('money') >= 500 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'You already have the maximum transportable of magazines!')
		else
    xPlayer.removeMoney(500)
    xPlayer.addInventoryItem('clip', 1)
    TriggerClientEvent('esx:showNotification', _source, 'You purchased a clip.')
	end
  else
    TriggerClientEvent('esx:showNotification', _source, _U('not_enough'))
  end
	...
	if sourceItem.limit ~= -1 and (sourceItem.count + 1) > sourceItem.limit then
		TriggerClientEvent('esx:showNotification', _source, 'You already have the maximum transportable of clips!')
	else
