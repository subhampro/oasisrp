        ...
        while true do
            ...
            for i = 1, #OwnedKeys, 1 do
                if GetDistanceBetweenCoords(pedcoords, UTK[OwnedKeys[i].motelId].rooms[OwnedKeys[i].roomId].stash, true) <= 4.5 then
                    DrawText3D(UTK[OwnedKeys[i].motelId].rooms[OwnedKeys[i].roomId].stash[1], UTK[OwnedKeys[i].motelId].rooms[OwnedKeys[i].roomId].stash[2], UTK[OwnedKeys[i].motelId].rooms[OwnedKeys[i].roomId].stash[3], "~r~Eşya Dolabı", 0.40)
                ...
                if #RoomMateData ~= 0 then
                    ...
                    for i = 1, #RoomMateData, 1 do
                        if GetDistanceBetweenCoords(pedcoords, UTK[RoomMateData[i].motelId].rooms[RoomMateData[i].roomId].stash, true) <= 4.5 then
                            DrawText3D(UTK[RoomMateData[i].motelId].rooms[RoomMateData[i].roomId].stash[1], UTK[RoomMateData[i].motelId].rooms[RoomMateData[i].roomId].stash[2], UTK[RoomMateData[i].motelId].rooms[RoomMateData[i].roomId].stash[3], "~r~Eşya Dolabı", 0.40)
                ...
                if GetDistanceBetweenCoords(Config.elevators[i][1].x, Config.elevators[i][1].y, Config.elevators[i][1].z, pedcoords, false) <= 50.0 then
                    if currentfloor == nil then
                        ...
                        if GetDistanceBetweenCoords(Config.elevators[i][currentfloor].x, Config.elevators[i][currentfloor].y, Config.elevators[i][currentfloor].z, pedcoords, true) <= 3.0 then
                            DrawText3D(Config.elevators[i][currentfloor].x, Config.elevators[i][currentfloor].y, Config.elevators[i][currentfloor].z, "[~r~E~w~] Elevator-"..currentfloor, 0.40)
...
function OpenReception(motelId)
    ...
    ESX.TriggerServerCallback("utk_motels:getReceptionInfos", function(cdata)
        ...
        local firstel =  {
            {label = "Total Rooms: "..#UTK[motelId].rooms, value = "aa"},
            {label = "Manage", value = "manage"},
            {label = "Rent Room", value = "rent"},
            {label = "Empty Room Inventory", value = "emptyRoom"},
        }
        ...
        ESX.UI.Menu.Open("default", "utk_motels", "reception", {
            ...
            if data.current.value == "manage" then
                ...
                for i = 1, #cdata.full, 1 do
                    table.insert(el, {label = "Room-"..cdata.full[i].roomId.." | Tenant: "..cdata.full[i].holdername, id = cdata.full[i].roomId, holder = cdata.full[i].holder, phone = cdata.full[i].phone})
                end
                ESX.UI.Menu.Open("default", "utk_motels", "manage-motel", {
                    title = "Total Full Rooms: "..#cdata.full,
                    align = "top-left",
                    elements = el
                    ...
                    if selection.id ~= nil then
                        ESX.UI.Menu.Open("default", "utk_motels", "room-info", {
                            ...
                            align = "top-left",
                            elements = {
                                {label = "Phone Number: "..selection.phone, value = "info"},
                                {label = "Change Keys", value = "change"},
                                {label = "Send Message", value = "notify"}
                            }
                            ...
                            elseif data3.current.value == "notify" then
                                ...
                                if text == nil or text:len() <= 10 then
                                    exports.mythic_notify:SendAlert("error", "You can't send empty or short message!", 6000)
                                    menu3.close()
                                    return
                                end
            ...
            elseif data.current.value == "rent" then
                ...
                for i = 1, #cdata.empty, 1 do
                    table.insert(el, {label = "Room-"..cdata.empty[i].roomId, value = cdata.empty[i].roomId})
                end
                ESX.UI.Menu.Open("default", "utk_motels", "rent-menu", {
                    title = "Total Empty Rooms: "..#cdata.empty,
                    align = "top-left",
                    elements = el
                }, function(data2, menu2)
                    local selection = data2.current
                    ...
                    if selection.value ~= nil then
                        ...
                        if #players >= 1 then
                            ...
                            exports.mythic_notify:SendAlert("error", "No players nearby.", 6000)
                        end
                    end
                end, function(data2, menu2)
                    menu2.close()
            ...
            elseif data.current.value == "debt" then
                ...
                if UTK[motelId].info.auto_pay then
                    table.insert(el, {label = "Auto-Pay: On", value = "toggle-auto"})
                else
                    table.insert(el, {label = "Auto-Pay: Off", value = "toggle-auto"})
                end
                if UTK[motelId].info.debt >= 0 then
                    table.insert(el, {label = "Pay Debt", value = "pay-debt"})
                end
                ESX.UI.Menu.Open("default", "utk_motels", "debt-menu", {
                    title = "Current Debt: "..UTK[motelId].info.debt,
                    align = "top-left",
                    elements = el
                }, function(data2, menu2)
                        ...
                        if tonumber(amount) ~= nil then
                            if tonumber(amount) <= UTK[motelId].info.debt then
                                ESX.TriggerServerCallback("utk_motels:payDebt", function(result)
                                    if result == true then
                                        ESX.UI.Menu.CloseAll()
                                        exports.mythic_notify:SendAlert("success", "You have paid "..amount.."$ of debt!", 5000)
                                        Citizen.Wait(1000)
                                        OpenReception(motelId)
                                    else
                                        menu2.close()
                                        exports.mythic_notify:SendAlert("error", "You don't have enough money in your bank account!", 5000)
                                    end
                                end, motelId, tonumber(amount))
                            else
                                menu2.close()
                                exports.mythic_notify:SendAlert("error", "You can't pay more then you owe!", 5000)
                            end
                        else
                            menu2.close()
                            exports.mythic_notify:SendAlert("error", "Invalid type!", 5000)
                        end
                    end
                end, function(data2, menu2)
                    menu2.close()
...
function OpenAutonomous(motelId)
    ...
    if not hasRoom then
        elements = {{label = "Rent room", value = 1}}
    elseif hasRoom then
        elements = {{label = "Unrent room", value = 2}}
    end
...
function RentRoom(motelId)
    ESX.TriggerServerCallback("utk_motels:getEmptyRoom", function(result)
        ...
        elseif not result then
            exports.mythic_notify:SendAlert("error", "There are no empty room in "..UTK[motelId].info.name..".", 6000)
        end
    end, motelId)
end
...
function OpenStash(motelId, roomId)
    ...
    elseif Config.inventorysystem == "disc" then
        --local inv = {
            --type = "utk",
            --owner = motelId.."_"..roomId
        --}
        --TriggerEvent("utk_motels:openDiscRoom", inv)
		TriggerEvent('disc-inventoryhud:openInventory', {
			type = 'motel',
			owner = motelId_roomId,
			slots = 80,
		})
    ...
    if not action then
        for i = 1, #OwnedKeys, 1 do
            if OwnedKeys[i].motelId == motelId and OwnedKeys[i].roomId == roomId then
                ...
                exports.mythic_notify:SendAlert("error", "You gave your key away.", 5000)
                break
            end
        end
    elseif action then
        table.insert(OwnedKeys, {motelId = motelId, roomId = roomId, roommate = roommate})
        exports.mythic_notify:SendAlert("success", UTK[motelId].info.name.." Room-"..roomId.."'s key is now yours.", 6500)
    end
end)
    ...
    if not action then
        for i = 1, #OwnedKeys, 1 do
            if OwnedKeys[i].motelId == motelId and OwnedKeys[i].roomId == roomId then
                ...
                exports.mythic_notify:SendAlert("error", "You no longer have a roommate for Room-"..roomId.." in "..UTK[motelId].info.name..".", 5000)
                break
            end
        end
    elseif action then
        for i = 1, #OwnedKeys, 1 do
            if OwnedKeys[i].motelId == motelId and OwnedKeys[i].roomId == roomId then
                ...
                exports.mythic_notify:SendAlert("success", "You now have a roommate for Room-"..roomId.." in "..UTK[motelId].info.name..".", 5000)
                break
            end
        end
    end
...
AddEventHandler("utk_motels:updateRoommates", function(motelId, roomId, action)
        ...
        for i = 1, #RoomMateData, 1 do
            if RoomMateData[i].motelId == motelId then
                if RoomMateData[i].roomId == roomId then
                    ...
                    exports.mythic_notify:SendAlert("error", "You have been kicked out of Room-"..roomId.." in "..UTK[motelId].info.name..".", 8000)
                    break
                end
            end
        end
    elseif action then
        ...
        exports.mythic_notify:SendAlert("success", "You are the roommate for Room-"..roomId.." in "..UTK[motelId].info.name..".", 6500)
    end
end)
...
RegisterCommand(Config.keyscommand, function()
    if #OwnedKeys ~= 0 or #RoomMateData ~= 0 then
        local el = {}
        for i = 1, #OwnedKeys, 1 do
            table.insert(el, {label = UTK[OwnedKeys[i].motelId].info.name.." | Room-"..OwnedKeys[i].roomId, owned = true, value = {m = OwnedKeys[i].motelId, r = OwnedKeys[i].roomId, mate = OwnedKeys[i].roommate}})
        end
        for m = 1, #RoomMateData, 1 do
            table.insert(el, {label = UTK[RoomMateData[m].motelId].info.name.." | Room-"..RoomMateData[m].roomId, owned = false, value = {m = RoomMateData[m].motelId, r = RoomMateData[m].roomId, mate = nil}})
        end
        receptionOpen = false
        ESX.UI.Menu.CloseAll()
            ...
            if data.current.value ~= nil then
                if Config.roommate then
                    if data.current.owned == true then
                        table.insert(firstel, {label = "Roommate options", value = "roommate"})
                    else
                        table.insert(firstel, {label = "Throw keys away", value = "throw"})
                    end
                end
                ESX.UI.Menu.Open("default", "utk_motels", "o-keys-2", {
                    title = UTK[data.current.value.m].info.name.." | Room-"..data.current.value.r,
                    align = "top-left",
                    elements = firstel
                }, function(data2, menu2)
                    ...
                    elseif data2.current.value == "roommate" then
                        ...
                        if data.current.value.mate ~= nil and data.current.value.mate ~= "" then
                            el2 = {{label = "Kick roommate", value = "kick"}}
                        else
                            el2 = {{label = "Set new roommate", value = "set"}}
                        end
                        ESX.UI.Menu.Open("default", "utk_motels", "manage-mate", {
                            ...
                            if data3.current.value == "set" then
                                ...
                                if #nearbyplayers >= 1 then
                                    ...
                                    mythic_notify:SendAlert("error", "No players nearby.", 5000)
                                end
...
RegisterCommand(Config.doorlockcommand, function()
    if #OwnedKeys == 0 and #RoomMateData == 0 then
        exports.mythic_notify:SendAlert("error", "You don't have any keys.", 4000)
        return
    end
    if #OwnedKeys > 0  then
        local pedcoords = GetEntityCoords(PlayerPedId())
