...
ComputerTick = function(currentProp)
    local PurchaseProp = function()
        ...
        ESX.TriggerServerCallback("james_motels:purchaseFurnishing", function(purchased)
            ...
            if purchased then
                ESX.ShowNotification("Purchase succeeded, delivered to your room.")
            else
                ESX.ShowNotification("Authentication failed, check your balance.")
            end
    ...
    Citizen.CreateThread(function()
        while DoesEntityExist(currentProp["handle"]) do
            ...
            DrawScriptText((cachedData["furnishingProp"]["coords"] or Config.MegaMall["object"]["pos"]) - vector3(0.0, 0.0, 0.25), "[~g~G~s~] to authenticate purchase.")
